...
class NodeHA:
    ...
    def __init__(self,x,y,nim,goalY,goalX,cost,chose):
        self.x = x
        self.y = y
        self.pirent = None
        self.childerin = list()
    ...
    def MoveToRight(self):
        ...
        if(self.y+1<len(self.data[self.x]) and self.data[self.x][self.y+1]==1):
            right = NodeHA(self.x,self.y+1,self.data,self.goalX,self.goalY,self.cost+1,self.chose)
    ...
    def MoveToLift(self):
        ...
        if(self.y-1>=0 and self.data[self.x][self.y-1]==1):
            Lift = NodeHA(self.x,self.y-1,self.data,self.goalX,self.goalY,self.cost+1,self.chose)
    ...
    def MovetoUp(self):
        if(self.x-1>=0 and self.data[self.x-1][self.y]==1):
            Up = NodeHA(self.x-1,self.y,self.data,self.goalX,self.goalY,self.cost+1,self.chose)
    ...
    def MoveToDown(self):
        ...
        if(self.x+1<len(self.data) and self.data[self.x+1][self.y]==1):
            Down = NodeHA(self.x+1,self.y,self.data,self.goalX,self.goalY,self.cost+1,self.chose)
